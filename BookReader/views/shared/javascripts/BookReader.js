...
BookReader.prototype.buildShareDiv = function(jShareDiv)
{
    ...
    var jForm = $([
        '<p>Pour partager ce document, sélectionnez une des options suivantes.</p>',
        '<form method="post" action="">',
            '<fieldset>',
                '<label for="pageview">Lien vers cette page:</label>',
                '<input type="text" name="pageview" id="pageview" value="' + pageView + '"/>',
            '</fieldset>',
            '<fieldset>',
                '<label for="booklink">Lien vers ce document:</label>',
                '<input type="text" name="booklink" id="booklink" value="' + bookView + '"/>',
            '</fieldset>',
            '<fieldset>',
                '<label for="iframe">Lorsque vous avez fini votre sélection, copiez-collez le code d\'intégration ci-dessous :</label>',
                '<fieldset class="sub">',
                    '<label class="sub">',
                    ...
                    '<label class="sub">',
                        ...
                        'Ouvrir à partir de cette page ?',
                    '</label>',
                '</fieldset>',
                '<textarea cols="30" rows="4" name="iframe" class="BRframeEmbed"></textarea>',
                '<p class="meta"><strong>NOTE:</strong> Le code a été testé sur la majorité des plate-formes de blogs (Blogger, WordPress, etc).</p>',
            '</fieldset>',
            '<fieldset class="center">',
                '<button type="button" onclick="$.fn.colorbox.close();">Fermer</button>',
            '</fieldset>',
        '</form>'].join('\n'));
