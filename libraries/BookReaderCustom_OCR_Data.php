...
class BookReader_Custom
{
    ...
    {
        return self::_getDataOfType($item, 'leaves');
    }
    ...
    {
        return self::_getDataOfType($item, 'nonLeaves');
    }
    ...
    {
        return self::_getDataOfType($item, 'pageIndexes');
    }
    ...
    {
        return self::_getDataOfType($item, 'pageNumbers');
    }
    ...
    {
        return self::_getDataOfType($item, 'pageLabels');
    }
    ...
    {
        return self::_getDataOfType($item, 'titleLeaf');
    }
    ...
    {
        return self::_getDataOfType($item, 'hasDataForSearch');
    }
    ...
    {
        $element = get_db()->getTable('Element')->findByElementSetNameAndElementName('Internal', 'BookReader Data');
        if (!$element) {
            return false;
        }

        // Remove current element if it exists.
        $element = $item->getElement('Internal', 'BookReader Data');
        $item->deleteElementTextsByElementId(array($element->id));

        // Get and simplify data.
        $data = self::getData($item);

        // Save data.
        if ($item->fileCount() > 0) {
            $saveData = $data;
            foreach ($saveData['leaves'] as &$leaf) {
                $leaf = $leaf ? $leaf->id : null;
            }
            foreach ($saveData['nonLeaves'] as &$leaf) {
                $leaf = $leaf ? $leaf->id : null;
            }

            $item->addTextForElement($element, json_encode($saveData), false);
            $item->save();
        }
    ...
    /**
     * Get all BookReader data for an item via code. This is used when data
     * are not saved.
     *
     * @return array
     *   Array of data.
    ...
    {
        $leaves = self::_getLeaves($item);
        $nonLeaves = self::_getNonLeaves($item);
        $pageIndexes = self::_getPageIndexes($item);
        $pageNumbers = self::_getPageNumbers($item);
        $pageLabels = self::_getPageLabels($item);
        list($pageWidths, $pageHeights) = BookReader::getImagesSizes($item, 'fullsize', $leaves);
        $titleLeaf = self::_getTitleLeaf($item);
        $hasDataForSearch = self::_hasDataForSearch($item);

        // This works even for an item without file.
        return array(
            'leaves' => $leaves,
            'nonLeaves' => $nonLeaves,
            ...
            'titleLeaf' => $titleLeaf,
            'hasDataForSearch' => $hasDataForSearch,
        );
    }

    /**
     * Get a specific BookReader data for an item.
     *
     * @return data
     */
    protected static function _getDataOfType($item, $type)
    {
        static $itemsData = array();

        if (!isset($itemsData[$item->id])) {
            $data = $item->getElementTexts('Internal', 'BookReader Data');
            if ($data) {
                $data = json_decode($data[0]->text, true);
                // get_records() function doesn't use a single query.
                foreach ($data['leaves'] as &$leaf) {
                    $leaf = $leaf ? get_record_by_id('File', $leaf) : null;
                }
                foreach ($data['nonLeaves'] as &$leaf) {
                    $leaf = $leaf ? get_record_by_id('File', $leaf) : null;
                }
                $itemsData[$item->id] = $data;
            }
            // If data are not available, they are saved the first time they are
            // used.
            else {
                $data = self::saveData($item);
                if ($data === false) {
                    $data = self::getData($item);
                }
                $itemsData[$item->id] = $data;
        ...
        return isset($itemsData[$item->id][$type])
            ? $itemsData[$item->id][$type]
            : null;
    }
     ...
     *   Array of filenames associated to original filenames.
     */
    protected static function _getLeaves($item)
    {
     ...
     *   Array of filenames associated to original filenames.
     */
    protected static function _getNonLeaves($item)
    {
     ...
     * @return array of integers
     */
    protected static function _getPageIndexes($item)
    {
        $leaves = self::_getLeaves($item);
        $indexes = array();
        $first = true;
     ...
     * @return array of strings
     */
    protected static function _getPageNumbers($item)
    {
        $leaves = self::_getLeaves($item);
        $numbers = array();
     ...
     * @return array of strings
     */
    protected static function _getPageLabels($item)
    {
        $leaves = self::_getLeaves($item);
        // Don't add a label if this is the same than the number.
        $numbers = self::_getPageNumbers($item);
        $labels = array();
        foreach ($leaves as $key => $leaf) {
            if (empty($leaf)) {
                $label = __('Blank page');
     ...
     *   Index for bookreader.
     */
    protected static function _getTitleLeaf($item)
    {
        $db = get_db();
        ...
        if ($result) {
            ...
            return BookReader::getLeafIndex($file, self::_getLeaves($item));
        }
     ...
     *   True if there are data for search, else false.
     */
    protected static function _hasDataForSearch($item)
    {
